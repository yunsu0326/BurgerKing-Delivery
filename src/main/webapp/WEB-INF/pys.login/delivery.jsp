...
function cartCount(){
	...
	$.ajax({
		   ...
		   success:function(json){
			   if(Number(json.cnt) == 1){
	               $("#secondtbl > tbody > tr:nth-child(1) > td:nth-child(2)").html(`<span class='badge badge-pill badge-danger'>현재 장바구니 \${json.cnt}개</span>`);
	               $("#secondtbl > tbody > tr:nth-child(2) > td").html(`<span>\${json.pname} \${Number(json.cnt)}개</span>`);
	            }
	            
	            else if(Number(json.cnt) > 1){
	               $("#secondtbl > tbody > tr:nth-child(1) > td:nth-child(2)").html(`<span class='badge badge-pill badge-danger'>현재 장바구니 \${json.cnt}개</span>`);
...
function deliveryCount(){
	...
	$.ajax({
		   url:"<%=ctxPath%>/shop/deliveryCount.bk",
		   type:"post",
		   data:{"userid":"${sessionScope.loginuser.userid}"},
		   dataType: "json",
		   success:function(json){
			   if(Number(json.cnt) == 1){
	               $("#firsttbl > tbody > tr:nth-child(1) > td:nth-child(2)").html(`<span class='badge badge-pill badge-danger'>오늘 주문내역 \${json.cnt}개</span>`);
	               $("#firsttbl > tbody > tr:nth-child(2) > td").html(`<span>\${json.pname} \${Number(json.cnt)}개</span>`);
	            }
	            
	            else if(Number(json.cnt) > 1){
	               $("#firsttbl > tbody > tr:nth-child(1) > td:nth-child(2)").html(`<span class='badge badge-pill badge-danger'>오늘 주문내역 \${json.cnt}개</span>`);
	               $("#firsttbl > tbody > tr:nth-child(2) > td").html(`<span>\${json.pname} 외 \${Number(json.cnt) - 1}개 </span>`);
	            }
